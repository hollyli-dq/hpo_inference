...
# Now unpack the normalized percentages:
    ...
    for iteration in range(1,num_iterations+1):
        ...
        if r < thresh_rho:
            ...
            log_acceptance_ratio =  (log_prior_proposed ) - (log_prior_current ) - math.log(delta)
        ...
        # ---- B) Update noise parameter ----
            ...
            llk_start = time.time()     
            log_llk_proposed = log_llk_current   ## change u0 will not change likelihood thus we can use the same log_llk_currentS
            dt = time.time() - llk_start
            total_likelihood_time = dt
            ...
            if rng.random() < min(1.0, np.exp(min(log_acceptance_ratio,700))):
                U_a_dict = U_a_dict_prime
                h_U = h_U_prime
                num_acceptances += 1
                acceptance_decisions.append(1)
